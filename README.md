# Tatetí de 3 fichas

Versión de tatetí hecha con HTML, CSS y JavaScript.

## Regla especial

Cada jugador solamente puede tener 3 fichas activas al mismo tiempo. Cuando pone la cuarta, desaparece la ficha más antigua de ese mismo jugador.

## Cómo ejecutarlo

Como es una app estática, podés abrir `index.html` directamente en el navegador o levantar un servidor simple.

### Opción 1: abrir el archivo

Abrí `index.html` con tu navegador.

### Opción 2: usar un servidor local con Python

```bash
python3 -m http.server 8000
```

Después abrí esta URL:

```text
http://localhost:8000
```
